...
def _convert_message_to_dict(message: BaseMessage, max_length_input: Optional[int] = None) -> dict:
    ...
    Returns:
        The dictionary.
    """
    prompt_tokens = gpt_tokenizer.encode(message.content)
    if max_length_input:
        prompt = gpt_tokenizer.decode(prompt_tokens[: max_length_input])
    else:
        prompt = gpt_tokenizer.decode(prompt_tokens)
...
class ChatSnowflakeCortex(BaseChatModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        message_dicts = [_convert_message_to_dict(m, self.max_tokens) for m in messages]
